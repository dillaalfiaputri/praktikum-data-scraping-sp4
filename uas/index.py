    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for berita in daftar_berita[:100]:
                ...
                cursor.execute(
                    "SELECT id FROM tbl_berita WHERE judul = %s",
                    (judul_asli,)
                )
                if cursor.fetchone():
                ...
                try:
                    ...
                    # Hapus elemen script dan style agar tidak dianggap teks berita
                    for element in soup_detail(['script', 'style']):
                        element.decompose()

                    # DetikInet biasanya menggunakan class detail__body-text
                ...
                sql_raw = (
                    ...
                    "(judul, url_link, url_gambar, isi_berita, tanggal_scraping, kategori) "
                    "VALUES (%s, %s, %s, %s, %s, %s)"
                )
                val_raw = (judul_asli, url_link, img_url, isi_berita, waktu_sekarang, kategori)
                cursor.execute(sql_raw, val_raw)

                # Simpan data hasil cleaning ke tabel hasil_cleaning
                sql_clean = (
                    "INSERT INTO hasil_cleaning "
                    "(judul_asli, judul_clean, url_link, url_link_clean, "
                    "url_gambar, waktu_scraping, isi_berita) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s)"
                )
                val_clean = (
                    judul_asli, judul_clean, url_link, url_link_clean,
                    img_url, waktu_sekarang, isi_berita
                )
...
def jalankan_scraper_otomatis():
    ...
    try:
        ...
        with open(path_log, "a") as log:
            log.write(
                f"[{datetime.now()}] OTOMASI SUKSES: "
                f"Berhasil mengarsipkan {inserted} berita baru.\n"
            )
    except Exception as e:
        with open(path_log, "a") as log:
